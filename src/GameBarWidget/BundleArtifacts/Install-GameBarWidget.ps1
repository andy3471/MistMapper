# Sideload the SC Bridge Game Bar widget (run elevated).
...
$old = Get-AppxPackage -Name 'SteamControllerBridge.GameBar' -ErrorAction SilentlyContinue
if ($old) {
...
$pkg = Get-AppxPackage -Name 'SteamControllerBridge.GameBar' -ErrorAction SilentlyContinue
...
Write-Host '1) Start SteamControllerBridge.exe (tray host)'
Write-Host '2) Press Win+G → Widget menu → pin "SC Bridge"'
